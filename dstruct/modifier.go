...
func (dm *DynamicStructModifierImpl) Instance() any {
	return helper.GetUnderlyingPointerValue(dm.strct)
}
...
func (dm *DynamicStructModifierImpl) Set(field string, value any) error {
	...
	if !CanExtend(value) {
		...
		s := time.Now()
		ExtendStruct(value).Build().Instance()
		e := time.Now()
		fmt.Println("TIME MINE: ", e.Sub(s).Nanoseconds())

		s = time.Now()
		reflect.ValueOf(value).Convert(fieldValue.Type())
		e = time.Now()

		fmt.Println("TIME reflect: ", e.Sub(s).Nanoseconds())
...
func (dm *DynamicStructModifierImpl) getFieldMap() FieldMap {
	return dm.fieldMap
}
