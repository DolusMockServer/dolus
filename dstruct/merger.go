...
func mergeStructs(left, right Builder, parentKind reflect.Kind) (any, error) {
	// currentElementDStruct := ExtendStruct(currentElement).Build()
	// currentElementDynamicStructFields := currentElementDStruct.getFieldMap()
	// newElementDynamicStructFields := ExtendStruct(newElement).Build().getFieldMap()

	// struct to be returned
	newStruct := ExtendStruct(left.Build().Instance())

	for name, field := range right.getRoot().children {
		// if v.fqn != v.GetFieldName() {
		// 	continue
		// }

		elementName := field.data.Name
		cV := left.getRoot().GetNode(name) //currentElementDynamicStructFields[k]
		// fullFieldName :=
		// if root != "" {
		...
		if field.data.Value.Kind() == reflect.Slice {
			vSliceType := helper.GetSliceType(field.data.Value)
			cVSliceType := helper.GetSliceType(cV.data.Value)
